...
-- USER CONFIG:
local jsfx_name = "JS: MIDI Global Transpose Slave (with Independent Octave)"  -- JSFX to add to selected tracks
local master_jsfx = "JS: MIDI Global Transpose Master" -- Master JSFX to check/add
...
for i = 0, num_sel - 1 do
  ...
  if not TrackHasFX(track, jsfx_name) then
    if track then
